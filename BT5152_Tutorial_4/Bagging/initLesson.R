...
# model_rpart <- rpart(price ~., training)
# out_train <- predict(model_rpart, training)
# rpart_mae_train <- mae(training$price, out_train)
# out <- predict(model_rpart, test)
...
train_bagging_expected <- function(n_bags, training_data){
...
# models <- train_bagging_expected(25, training)
# out_bagging_train <- predict_bagging_expected(models, training)
# bagging_mae_train <- mae(training$price, out_bagging_train)
# out_bagging <- predict_bagging_expected(models, test)
